...
function parseArgs(): Config {
  ...
  const config: Config = {
    userCount: 5,
    timeout: 60 * 1000, // 120 seconds - increased for XMTP operations
    env: XMTP_ENV,
  };
...
async function runStressTest(config: Config): Promise<void> {
  ...
  const promises = workers.map(async (worker, i) => {
    try {
      ...
      console.log(`✅ Worker ${i}: NewDM=${newDmTime}ms, Send=${sendTime}ms`);
      return { success: true, newDmTime, sendTime, responseTime: 0 };
    } catch (error) {
      console.error(`❌ Worker ${i} failed:`, error);
      return { success: false, newDmTime: 0, sendTime: 0, responseTime: 0 };
    }
  });
  ...
  try {
    ...
    console.log(
      `📊 Results: ${successful.length}/${
        ...
        (successful.length / config.userCount) * 100
      )}%)`
    );
    ...
    if (successful.length > 0) {
      ...
      console.log(
        `📈 Averages: NewDM=${Math.round(avgNewDm)}ms, Send=${Math.round(
          avgSend
        )}ms`
      );
    }
    ...
    if (successful.length > 0) {
      ...
      console.log(
        `📈 Averages (from ${successful.length} successful): NewDM=${Math.round(
          avgNewDm
        )}ms, Send=${Math.round(avgSend)}ms`
      );
    } else {
